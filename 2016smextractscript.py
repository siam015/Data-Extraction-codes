...
df = pd.DataFrame(0.0, columns = ['Year','Month','Day','Time','Surface_SM','Rootzone_SM','SM_Profile','Surface_Temperature'],index = date_range)
...
for tm in Date_range_list:
    ...
    df.loc[looptime]['SM_Profile'] = sm_profile[idx_lat_cumilla,idx_lon_cumilla]
    
    df.loc[looptime]['Surface_Temperature'] = surface_temp[idx_lat_cumilla,idx_lon_cumilla]



df.to_csv('SMAP_2016_Cumilla_Data.csv')
